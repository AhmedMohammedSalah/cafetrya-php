...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete'])) {
     ...
     if (isset($_POST['toggle_availability'])) {
        $productId = $_POST['product_id'];
        if (changeAvailability($productId)) {
            $_SESSION['message'] = 'Availability updated successfully.';
            $_SESSION['message_type'] = 'success';
            header("Location: listProducts.php?page=$currentPage");
            exit();
        } else {
            $_SESSION['message'] = 'Failed to update availability.';
 ...
 <div class="sidebar">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <span class="badge <?= $product['availability'] == 1 ? 'badge-available' : 'badge-unavailable' ?>">
    ...
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Set the product ID in the modal's hidden input
            document.getElementById('deleteProductId').value = productId;
        });
    });
    
    // Optional: Focus the cancel button when modal opens for better keyboard navigation
